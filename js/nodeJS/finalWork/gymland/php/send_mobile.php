...
//**********************************************
if(empty($_POST['btn_mobile'])){
	if($_POST['user_mail_mobile'] != ''){

		$to = 'admin@gymland.ru'; 
        $subject = 'Подписка на акции клиентом'.$user_mail_mobile; 
  ...
  $mailto=mail($to, $subject, $message, $headers);
		if($mailto == true){
           ...
           if (!preg_match("|^[-0-9a-z_\.]+@[-0-9a-z_^\.]+\.[a-z]{2,6}$|i",$_POST['user_mail_mobile'])){
           echo 2;
           }
		}else{
