...
app.set('views', __dirname + '/views'); app.engine('mustache', mustacheExpress());
...
app.use(session({
    secret: 'keyboard cat', //подпись сессионной cookie
    name: 'sid', //имя сессионной cookie
    cookie: { httpOnly: true, maxAge: 60000}, //настройка cookie, в частности время жизни 1 мин
    resave: true, //на всякий случай, используется для хранилищ без функции touch
    saveUninitialized: true //сохранение объекта сессии в память
}));
//подключаем статический сервер на папку public
app.use(express.static('public'));
//подключаем и запускаем настройку модуля passport
